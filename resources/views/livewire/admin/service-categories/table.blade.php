                ...
                @forelse ($data as $item)
                    <tr wire:key="service-category-{{ $item->id }}">
                        ...
                        <td style="cursor:pointer;"
                            ...
                            @else
                                <div class="user-info">
                                    <span class="text-muted" style="font-size: 0.8vw;">No providers</span>
                                </div>
                            @endif
                        </td>
    ...
    @if ($showFilterModal)
        <div class="modal filter-theme-modal" style="display: flex;">
            <div class="modal-content filter-modal">
                ...
                ]" placeholder="Select status" wireModel="tempStatus"
                    :value="$tempStatus" class="form-input mt-2" />
...
@push('scripts')
    ...
    <script>
        $(document).on('click', '.showDeleteModal', function(e) {
            ...
            e.stopPropagation();
            let id = $(this).data('id');
            $('.deleteModal').hide(); // Close others
            $('#globalDeleteModal' + id).show();
        })
        ...
        $(document).on('click', '.closeDeleteModal, .cancel-delete-btn', function(e) {
            ...
            e.stopPropagation();
            let id = $(this).data('id');
            $('#globalDeleteModal' + id).hide();
        })

        $(document).on('click', '.confirm-delete-btn', function(e) {
            // Modal will be hidden by Livewire refresh after delete
            e.stopPropagation();
        })
