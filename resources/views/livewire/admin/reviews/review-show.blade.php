<div>
            ...
            <div class="card border-0 p-3">
                ...
                <div class="d-flex align-items-center justify-content-between p-3 mb-3">
                    ...
                    <div class="status-dropdown position-relative">
                        @php
                            $statusClass = strtolower($review->status) === 'published' ? 'active' : (strtolower($review->status) === 'unpublished' ? 'inactive' : 'pending');
                        @endphp
                        ...
                        <ul class="dropdown-menu show-status-menu" style="display: none; position: absolute; right: 0; z-index: 100;">
                ...
                <div class="p-3 border rounded reviewbox" style="display: grid; row-gap: 22px;">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        ...
                        <div class="stars-rating d-flex">
                            @for($i = 1; $i <= 5; $i++)
                                @if($i <= $review->rating)
                                    <img src="{{ asset('assets/images/icons/star.svg') }}" alt="star" style="width:1.2vw; height:1.2vw; margin-right:0.3vw;">
                                @else
                                    <img src="{{ asset('assets/images/icons/empty_star.svg') }}" alt="empty star" style="width:1.2vw; height:1.2vw; margin-right:0.3vw;">
                                @endif
                            @endfor
                        </div>
                        <small class="" style="font-size: 0.9vw;color:#8E8E8E;">{{ $review->created_at->diffForHumans() }}</small>
                    </div>
                    ...
                    <div class="action-buttons d-flex justify-content-between align-items-center">
                        <div class="edit-delete-buttons d-flex gap-3">
                            @if(!$isEditing)
                                <!-- EDIT -->
                                <a href="javascript:void(0);" wire:click="toggleEdit" class="view-btn" style="color: grey; display: flex; align-items: center; gap: 5px; font-size: 0.8vw; text-decoration: none;">
                                    <img src="{{ asset('assets/images/icons/edit-2.svg') }}" style="width: 1vw; height: 1vw;">
                                    Edit
                                </a>

                                <!-- DELETE -->
                                <a href="javascript:void(0);" onclick="confirmDeleteReview()" class="view-btn" style="color: grey; display: flex; align-items: center; gap: 5px; font-size: 0.8vw; text-decoration: none;">
                                    <img src="{{ asset('assets/images/icons/trash-theme.svg') }}" style="width: 1vw; height: 1vw;">
                                    Delete
                                </a>
                            @else
                                <button wire:click="saveReview" class="btn btn-primary btn-sm" style="background-color: #004e42; border: none; padding: 0.4vw 1vw; font-size: 0.8vw;">
                                    Save changes
                                </button>
                                <button wire:click="toggleEdit" class="btn btn-secondary btn-sm" style="padding: 0.4vw 1vw; font-size: 0.8vw;">
                                    Cancel
                                </button>
                            @endif
                        </div>
    ...
    <!-- Delete Modal -->
    <div id="deleteReviewModal" class="deleteModal" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.4); justify-content: center; align-items: center; z-index: 1000;">
        <div class="delete-card" style="background: #fff; padding: 20px; border-radius: 10px; min-width: 300px;">
            <div class="delete-card-header d-flex justify-content-between align-items-center mb-3">
                ...
                <span class="delete-close" style="cursor:pointer; font-size: 1.5rem;" onclick="closeDeleteModal()">&times;</span>
            </div>
    ...
    <style>
        ...
        .status-btn.active { background-color: #e0f5e9; color: #28a745; }
        .status-btn.inactive { background-color: #fdecea; color: #dc3545; }
        .status-btn.pending { background-color: #fff3cd; color: #856404; }
        .dropdown-menu li { padding: 8px 12px; cursor: pointer; list-style: none; }
