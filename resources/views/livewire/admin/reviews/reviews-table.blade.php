<div>
    <livewire:admin.components.toolbar 
        label="reviews" 
        button_label="" 
        search_label="" 
                ...
                <thead>
                    <tr>
                        ...
                        <th class="sortable" wire:click="sortBy('sender_id')">Reviewer
                            <img src="{{ asset('assets/images/icons/sort.svg') }}" class="sort-icon {{ $sortField === 'sender_id' ? ($sortDirection === 'asc' ? '' : 'desc') : '' }}">
                        </th>
                        <th class="sortable" wire:click="sortBy('receiver_id')">Reviewed {{ $activeTab === 'users' ? 'Provider' : 'User' }}
                            <img src="{{ asset('assets/images/icons/sort.svg') }}" class="sort-icon {{ $sortField === 'receiver_id' ? ($sortDirection === 'asc' ? '' : 'desc') : '' }}">
                        </th>
                        <th class="sortable" wire:click="sortBy('review')"> Review 
                            <img src="{{ asset('assets/images/icons/sort.svg') }}" class="sort-icon {{ $sortField === 'review' ? ($sortDirection === 'asc' ? '' : 'desc') : '' }}">
                        </th>
                ...
                <tbody>
                    @forelse($reviews as $review)
                        <tr wire:key="review-{{ $review->id }}">
                            ...
                            <td>
                                <div class="user-info">
                                    <img src="{{ asset($review->reviewer->avatar ?? 'assets/images/icons/person-one.svg') }}" alt="User">
                                    <div>
                            ...
                            <td>
                                <div class="user-info">
                                    <img src="{{ asset($review->reviewedProvider->avatar ?? 'assets/images/icons/person-one.svg') }}" alt="User">
                                    <div>
                                        <span class="user-theme-name">{{ $review->reviewedProvider->name ?? 'N/A' }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="min-width-200">
                                <div class="stars-rating">
                                    @for($i = 1; $i <= 5; $i++)
                                        @if($i <= $review->rating)
                                            <img src="{{ asset('assets/images/icons/star.svg') }}" alt="star" style="margin-right: 0.1vw;">
                                        @else
                                            <img src="{{ asset('assets/images/icons/empty_star.svg') }}" alt="empty star" style="margin-right: 0.1vw;">
                                        @endif
                                    @endfor
                                </div>
                            ...
                            <td>
                                <div class="status-dropdown position-relative">
                                    @php
                                        $statusClass = strtolower($review->status) === 'published' ? 'publish' : (strtolower($review->status) === 'unpublished' ? 'unpublished' : 'pending');
                                    @endphp
                                    <span class="status {{ $statusClass }}" onclick="toggleReviewDropdown(this, {{ $review->id }})">
                                    ...
                                    <ul class="dropdown-menu review-status-menu-{{ $review->id }}" style="display:none; position: absolute; z-index: 100;">
                            ...
                            <td style="position:relative;">
                                <div class="actions-dropdown">
                                    <button class="actions-btn act" onclick="toggleReviewActions(this, {{ $review->id }})">
                                        <img src="{{ asset('assets/images/icons/three_dots.svg') }}" class="dots-img" alt="">
                                    </button>
                                    <div class="actions-menu review-actions-menu-{{ $review->id }}" style="display:none; position: absolute; right: 0; z-index: 100;">
                                        <a href="{{ route('reviews.show', ['id' => $review->id]) }}">
                                            <img src="{{ asset('assets/images/icons/eye.svg') }}" alt=""> View Details
                                        </a>
                                        <a href="javascript:void(0);" class="showReviewDeleteModal" data-id="{{ $review->id }}">
                                ...
                                <!-- ✅ Delete Modal -->
                                <div id="deleteReviewModal{{ $review->id }}" class="deleteModal" style="display: none; position: absolute; top: 2vw; right: 6vw; z-index: 1000;">
                                    <div class="delete-card">
                                        <div class="delete-card-header">
                                            <h3 class="delete-title">Delete review</h3>
                                            <span class="delete-close closeReviewDeleteModal" data-id="{{ $review->id }}">&times;</span>
                                        </div>
                                        <p class="delete-text">Are you sure you want to delete this review?</p>
                                        <div class="delete-actions justify-content-start">
                                            <button class="confirm-delete-btn" wire:click="delete({{ $review->id }})">Delete</button>
                                            <button class="cancel-delete-btn closeReviewDeleteModal" data-id="{{ $review->id }}">Cancel</button>
                                        </div>
                                    </div>
                                </div>
                            </td>
    ...
    @if ($showFilterModal)
        <div class="modal filter-theme-modal" style="display: flex;">
            <div class="modal-content filter-modal">
                ...
                ]" placeholder="Select status" wireModel="tempStatus" :value="$tempStatus" class="form-input mt-2" />
    ...
    <style>
        ...
        .theme-table tbody tr:hover { background-color: #f1f1f1; transition: background-color 0.2s ease; }
        .unpublished { color: #D00416; border: 1px solid #D00416; background-color: #fb374741 !important; }
        .publish { background-color: #e0f5e9 !important; color: #28a745 !important; border: 1px solid #28a745 !important; }
        .status.pending { background-color: #fff3cd !important; color: #856404 !important; border: 1px solid #ffeeba !important; }
        .status-dropdown .dropdown-menu { width: 100%; min-width: 120px; }
        .status-dropdown .dropdown-menu li { padding: 8px 12px; cursor: pointer; list-style: none; }
        .status-dropdown .dropdown-menu li:hover { background-color: #f8f9fa; }
    </style>

    <script>
        function toggleReviewDropdown(el, id) {
            const menu = document.querySelector('.review-status-menu-' + id);
        ...
        function toggleReviewActions(el, id) {
            const menu = document.querySelector('.review-actions-menu-' + id);
        ...
        $(document).on('click', '.closeReviewDeleteModal', function(e) {
            e.preventDefault();
            e.stopPropagation();
            let id = $(this).data('id');
        ...
        document.addEventListener('click', function(e) {
            if (!e.target.closest('.status-dropdown') && !e.target.closest('.actions-dropdown')) {
                document.querySelectorAll('.dropdown-menu, .actions-menu').forEach(m => m.style.display = 'none');
            }
            if (!$(e.target).closest('.deleteModal').length && !$(e.target).closest('.showReviewDeleteModal').length) {
                $('.deleteModal').hide();
            }
        });
    </script>
