@php
    ...
    $refLabel = $booking?->booking_ref ?? ('#' . $dispute->id);
@endphp
<!DOCTYPE html>
<html>
                                ...
                                <tr>
                                    <td width="40" style="width:40px; vertical-align:middle;">
                                        <svg width="30" height="37" viewBox="0 0 30 37" fill="none"
                                            ...
                                            <g clip-path="url(#clip0_dispute_pdf)">
                                                <path
                                            ...
                                            <defs>
                                                <clipPath id="clip0_dispute_pdf">
                                                    <rect width="30" height="36.8571" fill="white" />
                                                </clipPath>
                                            </defs>
                                        </svg>
                                    </td>
                                    <td style="color:#ffffff; font-weight:500; font-size:16px; line-height:1; vertical-align:middle;">
                                        {{ $brandName }}
                                    </td>
                                </tr>
                            </table>
                    ...
                    <tr>
                        <td style="padding:30px; color:#717171; font-weight:400; font-size:14px;">
                            ...
                            <p style="margin:0 0 15px 0;">
                                Dispute ID <strong style="color:#393939;">{{ $dispute->id }}</strong>
                                · Status <strong style="color:#393939;">{{ ucfirst($dispute->status) }}</strong>
                            </p>
                            ...
                            <div style="margin:0 0 15px 0; color:#717171;">
                                <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:15px; font-size:14px;">
                                    <tr>
                                        <td style="padding:0 0 8px 0; color:#717171; width:42%;">Raised</td>
                                        <td style="padding:0 0 8px 0; color:#393939;">{{ optional($dispute->created_at)->format('d M Y, h:i A') ?? '—' }}</td>
                                    </tr>
                                    <tr>
                                        <td style="padding:0 0 8px 0; color:#717171;">Generated</td>
                                        <td style="padding:0 0 8px 0; color:#393939;">{{ now()->format('d M Y, h:i A') }}</td>
                                    </tr>
                                </table>

                                @if($booking)
                                    <p style="margin:0 0 10px 0; font-size:16px; font-weight:600; color:#393939;">Booking details</p>
                                    <table width="100%" cellpadding="8" cellspacing="0" style="border:1px solid #e3e8ea; background:#ffffff; font-size:14px; margin-bottom:15px;">
                                        <tr>
                                            <td style="border-bottom:1px solid #eef1f3; color:#717171; width:38%;">Booking ID</td>
                                            <td style="border-bottom:1px solid #eef1f3; color:#393939;">{{ $booking->booking_ref ?? '—' }}</td>
                                        </tr>
                                        <tr>
                                            <td style="border-bottom:1px solid #eef1f3; color:#717171;">Date</td>
                                            <td style="border-bottom:1px solid #eef1f3; color:#393939;">{{ optional($booking->created_at)->format('d M, Y') ?? '—' }}</td>
                                        </tr>
                                        <tr>
                                            <td style="border-bottom:1px solid #eef1f3; color:#717171;">Time</td>
                                            <td style="border-bottom:1px solid #eef1f3; color:#393939;">{{ optional($booking->created_at)->format('h:i A') ?? '—' }}</td>
                                        </tr>
                                        <tr>
                                            <td style="border-bottom:1px solid #eef1f3; color:#717171;">Duration</td>
                                            <td style="border-bottom:1px solid #eef1f3; color:#393939;">{{ $booking->duration ?? '—' }}</td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            ...
                                            <td style="border-bottom:1px solid #eef1f3; color:#393939;">{{ optional($booking->service)->name ?? '—' }}</td>
                                        </tr>
                                        <tr>
                                            <td style="border-bottom:1px solid #eef1f3; color:#717171;">Service cost</td>
                                            <td style="border-bottom:1px solid #eef1f3; color:#0b4f43; font-weight:600;">${{ number_format($booking->total_price ?? 0, 2) }}</td>
                                        </tr>
                                        <tr>
                                            <td style="border-bottom:1px solid #eef1f3; color:#717171;">Booking status</td>
                                            <td style="border-bottom:1px solid #eef1f3; color:#393939;">{{ str_replace('_', ' ', ucfirst($booking->status ?? '')) }}</td>
                                        </tr>
                                        <tr>
                                            <td style="border-bottom:1px solid #eef1f3; color:#717171;">Service provider</td>
                                            <td style="border-bottom:1px solid #eef1f3; color:#393939;">{{ optional($booking->provider)->name ?? '—' }}</td>
                                        </tr>
                                        <tr>
                                            <td style="color:#717171;">Service user</td>
                                            <td style="color:#393939;">{{ optional($booking->customer)->name ?? '—' }}</td>
                                        </tr>
                                    </table>
                                @endif

                                <p style="margin:0 0 10px 0; font-size:16px; font-weight:600; color:#393939;">Dispute details</p>
                                <table width="100%" cellpadding="8" cellspacing="0" style="border:1px solid #e3e8ea; background:#ffffff; font-size:14px;">
                                    <tr>
                                        <td style="border-bottom:1px solid #eef1f3; color:#717171; width:38%;">Raised by</td>
                                        <td style="border-bottom:1px solid #eef1f3; color:#393939;">{{ optional($dispute->user)->name ?? '—' }}</td>
                                    </tr>
                                    <tr>
                                        <td style="border-bottom:1px solid #eef1f3; color:#717171;">Email</td>
                                        <td style="border-bottom:1px solid #eef1f3; color:#393939;">{{ optional($dispute->user)->email ?? '—' }}</td>
                                    </tr>
                                    <tr>
                                        <td style="border-bottom:1px solid #eef1f3; color:#717171; vertical-align:top;">Issue</td>
                                        <td style="border-bottom:1px solid #eef1f3; color:#393939; vertical-align:top;">{{ $dispute->message ?? '—' }}</td>
                                    </tr>
                                    <tr>
                                        <td style="color:#717171; vertical-align:top;">Attachment</td>
                                        <td style="color:#393939; vertical-align:top;">{{ $attachmentUrl }}</td>
                                    </tr>
                                </table>
                            </div>

                            <p style="margin:20px 0 0 0; font-size:12px; color:#8a9399; line-height:1.5;">
                                {{ $brandName }}
